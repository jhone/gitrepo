...
import com.redsun.platf.dao.tag.easyui.model.entity.TSRole;
import com.redsun.platf.dao.tag.easyui.model.json.ComboBox;
import com.redsun.platf.dao.tag.easyui.model.json.DataGrid;
import com.redsun.platf.dao.tag.vo.datatable.DataTableReturn;
import com.redsun.platf.dao.tag.vo.easyui.Autocomplete;
import com.redsun.platf.entity.tag.PagedList;
import com.redsun.platf.util.JsonUtils;
import com.redsun.platf.util.ReflectHelper;
...
public class TagUtil {
	 ...
	 * @param flds
	 */
	public static Object fieldNametoValues(String flds, Object o){
		Object value = "";
		String fieldName = "";
		...
		if (flds.indexOf("_") == -1) {
			...
			fieldName = flds.substring(0, flds.indexOf("_"));//外键字段引用名
			childFieldName = flds.substring(flds.indexOf("_") + 1);//外键字段名
		}
		value = reflectHelper.getMethodValue(fieldName)==null?"":reflectHelper.getMethodValue(fieldName);
		if (value !=""&&value != null && flds.indexOf("_") != -1) {
	...
	public static String getAutoList(Autocomplete autocomplete, List list) throws Exception {
		...
		jsonTemp = jsonTemp + "]}";
		return JsonUtils.toJsonString(jsonTemp).toString();
	}
	/**
...
//				jObject = //JSONObject.fromObject(
//                       JsonUtils.praseJson( listtojson(dg.getField().split(","), dg.getTotal(), dg.getReaults(),dg.getFooter().split(","))
//                );
//			}else{
//				jObject =  JsonUtils.praseJson(listtojson(dg.getField().split(","), dg.getTotal(), dg.getReaults(), null));
//			}
//			//update-end--Author:zhaojunfu  Date:20130520 for：TASK #109 datagrid标签没有封装合计功能
//		} catch (Exception e) {
//			e.printStackTrace();
...
//	}
	 ...
	 * 返回列表JSONObject对象
	 * @param field
	 * @param dataTable dataGrid
	 * @return
	 */
	private static String getJson(DataTableReturn dataTable,String field) {
        String jObject = null;
		try {
//			jObject = JSONObject.fromObject(datatable(field, dataTable.getiTotalDisplayRecords(), dataTable.getAaData()));
			jObject = JsonUtils.toJsonString(
                    datatable(field, dataTable.getiTotalDisplayRecords(), dataTable.getAaData())
            );
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	// JSON返回页面MAP方式
	public static void ListtoView(HttpServletResponse response, PagedList pageList) {
...
//		JSONObject jsonObject = JsonUtils.convert2JSON(map);
//        JsonGenerator jsonGenerator = mapper.getJsonFactory().createJsonGenerator(System.out, JsonEncoding.UTF8);
	 ...
	 * @param dg dataGrid
	 */
	public static void datagrid(HttpServletResponse response,DataGrid dg) throws IOException {
		response.setContentType("application/json");
		response.setHeader("Cache-Control", "no-store");
		String object = JsonUtils.toJsonString(dg);
		try {
	...
	public static void datatable(HttpServletResponse response, DataTableReturn dataTableReturn,String field) {
		response.setContentType("application/json");
		response.setHeader("Cache-Control", "no-store");
        String object = TagUtil.getJson(dataTableReturn,field);
		try {

...
public interface IPagedDao<T, PK extends Serializable> extends  IAbstractBaseDao<T, PK> {
	 ...
	 * 分页获取全部对象.
	 */
	public abstract PagedResult<T> getAll(final PagedResult<T> page);
	 ...
	 */
	public abstract PagedResult<T> findPage(final PagedResult<T> page, final String hql,
                                     final Object... values);
	 ...
	 * @return 分页查询结果.附带结果列表及所有查询输入参数.
	 */
	public abstract PagedResult<T> findPage(final PagedResult<T> page,

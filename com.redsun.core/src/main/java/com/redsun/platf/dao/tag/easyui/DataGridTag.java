...
import com.redsun.platf.dao.tag.easyui.model.entity.TSType;
import com.redsun.platf.dao.tag.easyui.model.entity.TSTypegroup;
import com.redsun.platf.dao.tag.vo.easyui.ColumnValue;
import com.redsun.platf.dao.tag.vo.easyui.DateGridColumn;
import com.redsun.platf.dao.tag.vo.easyui.DateGridUrl;
import com.redsun.platf.dao.tag.vo.easyui.OptTypeDirection;
import com.redsun.platf.sys.EPApplicationAttributes;
import com.redsun.platf.util.ResourceUtil;
...
public class DataGridTag extends TagSupport {
	...
	public void setConfUrl(String url, String title, String message, String exp,String operationCode) {
		...
		//----------------------------------------------------------------
        if(ResourceUtil.getSessionUserName().getLoginName().equals("admin")|| !EPApplicationAttributes.BUTTON_AUTHORITY_CHECK){
	...
	public void setOpenUrl(String url, String title, String width, String height, String exp,String operationCode) {
		...
		//update-start--Author:anchao  Date:20130415 for：按钮权限控制
		//----------------------------------------------------------------
		if(ResourceUtil.getSessionUserName().getLoginName().equals("admin")||
                !EPApplicationAttributes.BUTTON_AUTHORITY_CHECK){
			urlList.add(dateGridUrl);
		}else if(StringUtils.isNotEmpty(operationCode)){
			Set<String> operationCodes = (Set<String>) super.pageContext.getRequest().getAttribute("operationCodes");
			if (null!=operationCodes) {

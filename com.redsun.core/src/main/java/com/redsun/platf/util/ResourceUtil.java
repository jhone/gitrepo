...
import com.redsun.platf.dao.tag.easyui.model.common.SessionInfo;
...
public class ResourceUtil {
	...
	public static final UserAccount getSessionUserName() {
		HttpSession session = ContextHolderUtils.getSession();
		session.setMaxInactiveInterval(-1);
		if (session.getAttributeNames().hasMoreElements()) {
			SessionInfo sessionInfo1 = (SessionInfo) session.getAttribute(
                    EPApplicationAttributes.USER_SESSION);
	...
	public static final List<AccountRole> getSessionTSRoleFunction(String roleId) {
		HttpSession session = ContextHolderUtils.getSession();
		session.setMaxInactiveInterval(-1);
		if (session.getAttributeNames().hasMoreElements()) {
			List<AccountRole> TSRoleFunctionList = (List<AccountRole>)session.getAttribute(roleId);
	...
	public static final Map<Object, Object> getConfigMap(String path) {
		...
		return StringUtils.SetToMap(set);
	}
